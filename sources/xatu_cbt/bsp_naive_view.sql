-- Naive view: ready time vs post-snappy size, measured from slot start.
-- Bin individual-class node observations by post-snappy block size and report
-- median / p95 of seen_slot_start_diff (when each node first saw the block).
WITH blocks AS (
...
joined AS (
    SELECT
        s.seen_slot_start_diff AS seen_ms,
...
SELECT
    ...
    round(quantile(0.5)(seen_ms),  0) AS p50_seen_ms,
    round(quantile(0.95)(seen_ms), 0) AS p95_seen_ms,
    round(quantile(0.99)(seen_ms), 0) AS p99_seen_ms
FROM joined
GROUP BY bin_label
